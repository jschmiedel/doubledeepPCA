...
pairdistances_from_PDB = function(input_file,
  ...
  for (m in 1:M) {
    ...
    aaseq_PDB = unique(structure[,.(AA = convert_AAabr_one_three(as.character(unique(resname)))),by=.(resid,chainid)])
    ...
    if (m==1) {
      ...
      distance[,chainids := paste0(given_chainids,collapse=",")]
    }
    
    ### 2a) calculate minimal side-chain heavy atom distance
    structure_scHA = structure[union(intersect(grep(pattern="^[COSN][B-Z]$",elename),
                                               which(resname != "GLY")),intersect(grep(pattern="^CA$",elename),which(resname == "GLY")))]
    setkey(structure_scHA,resid)
    # X_DT = data.table(dist = c(as.matrix(pdist::pdist(X=as.matrix(structure_scHA[,.(x1,x2,x3)]),
    #                                                   Y=as.matrix(structure_scHA[,.(x1,x2,x3)])))),
    #                   Pos1 = rep(structure_scHA$resid,nrow(structure_scHA)),
    #                   Pos2 = rep(structure_scHA$resid,each = nrow(structure_scHA)))
    X_DT = data.table(dist = c(as.matrix(pdist::pdist(X=as.matrix(structure_scHA[chainid == two_chainids[1],.(x1,x2,x3)]),
                                                      Y=as.matrix(structure_scHA[chainid == two_chainids[2],.(x1,x2,x3)])))),
                      Pos1 = rep(structure_scHA[chainid==two_chainids[1],resid],structure_scHA[chainid==two_chainids[2],.N]),
                      Pos2 = rep(structure_scHA[chainid==two_chainids[2],resid],each = structure_scHA[chainid==two_chainids[1],.N]))
distance = merge(distance,X_DT[,.(X = min(dist)),.(Pos1,Pos2)],by=c("Pos1","Pos2"),all.x=T)
names(distance)[grep("^X$",names(distance))] = paste0("scHAmin",m)

### 2b) calculate minimal heavy atom distance
structure_HA = structure[grep(pattern="^[COSN]",elename)]
setkey(structure_HA,resid)
# X_DT = data.table(dist = c(as.matrix(pdist::pdist(X=as.matrix(structure_HA[,.(x1,x2,x3)]),
#                                                   Y=as.matrix(structure_HA[,.(x1,x2,x3)])))),
#                   Pos1 = rep(structure_HA$resid,nrow(structure_HA)),
#                   Pos2 = rep(structure_HA$resid,each = nrow(structure_HA)))
X_DT = data.table(dist = c(as.matrix(pdist::pdist(X=as.matrix(structure_HA[chainid == two_chainids[1],.(x1,x2,x3)]),
                                                  Y=as.matrix(structure_HA[chainid == two_chainids[2],.(x1,x2,x3)])))),
                  Pos1 = rep(structure_HA[chainid==two_chainids[1],resid],structure_HA[chainid==two_chainids[2],.N]),
                  Pos2 = rep(structure_HA[chainid==two_chainids[2],resid],each = structure_HA[chainid==two_chainids[1],.N]))
distance = merge(distance,X_DT[,.(X = min(dist)),.(Pos1,Pos2)],by=c("Pos1","Pos2"),all.x=T)
names(distance)[grep("^X$",names(distance))] = paste0("HAmin",m)

### 2c) calculate CB distances (use CA in case of Glycine)
structure_CB = structure[elename == "CB" | (elename == "CA" & resname == "GLY"),.(chainid,resid,x1,x2,x3)]
setkey(structure_CB,resid)
# X_DT = data.table(dist = c(as.matrix(pdist::pdist(X=as.matrix(structure_CB[,.(x1,x2,x3)]),
#                                                   Y=as.matrix(structure_CB[,.(x1,x2,x3)])))),
#                   Pos1 = rep(structure_CB$resid,nrow(structure_CB)),
#                   Pos2 = rep(structure_CB$resid,each = nrow(structure_CB)))
X_DT = data.table(dist = c(as.matrix(pdist::pdist(X=as.matrix(structure_CB[chainid == two_chainids[1],.(x1,x2,x3)]),
                                                  Y=as.matrix(structure_CB[chainid == two_chainids[2],.(x1,x2,x3)])))),
                  Pos1 = rep(structure_CB[chainid==two_chainids[1],resid],structure_CB[chainid==two_chainids[2],.N]),
                  Pos2 = rep(structure_CB[chainid==two_chainids[2],resid],each = structure_CB[chainid==two_chainids[1],.N]))
distance = merge(distance,X_DT[,.(X = min(dist)),.(Pos1,Pos2)],by=c("Pos1","Pos2"),all.x=T)
names(distance)[grep("^X$",names(distance))] = paste0("CB",m)
...
#          by=.(Pos1,Pos2)]
  ...
  contactmap = distance[between(Pos1,two_starts[1],two_ends[1]) & 
                          between(Pos2,two_starts[2],two_ends[2]),
