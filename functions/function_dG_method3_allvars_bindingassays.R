function_dG_method3_allvars_bindingassay <- function(
    ...
    for (Xval in 1:10) {
        ...
        models <- foreach(m = 1:n_bootstraps) %dopar% {
            ...
            global_model$par <- c(global_model$par, function_binding_F2dG(
                b_fitness = 1, s_dG = global_model$par[2 * id_L + 3],
        ...
        save(
            models, 
            file = paste0("processed_data/dG_method3_", dataset_name, "_", n_bootstraps, "models_", Xval, "Xval.Rdata")
        )

        # plot models
        function_dG_plot_models(
            models_name = 
                paste0("dG_method3_", dataset_name, "_", n_bootstraps, "models_", Xval, "Xval"),
