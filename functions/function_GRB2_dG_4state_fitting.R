function_GRB2_dG_4state_fitting <- function(
  parameters,
  fixed_par,
  ...
  if (sum(!is.na(fw_list$s_fitness)) > 0) { #is zero for method 3
    s_fpred <- function_dG_4state_folding_dg2fitness(
      s1_ddg = s1_ddg_var, 
      s2_ddg = s2_ddg_var, 
  ...
  ## binding phenotype
  b_fpred <- function_dG_4state_binding_dg2fitness(
    s1_ddg = s1_ddg_var,
    s2_ddg = s2_ddg_var,
    b_ddg = b_ddg_var,
  ...
  if (predict_binding0 == TRUE) {
    b0_fpred <- function_dG_4state_binding_dg2fitness(
      s1_ddg = s1_ddg_var,
      s2_ddg = s2_ddg_var, 
      b_ddg = 0,
      b_dgwt = parameters[all_idl + 1], 
