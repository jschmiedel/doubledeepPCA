function_GB1_dG_4state_estimation <- function(
  ...
  if (task == 1) {
    ...
    if (approach == 1) {
      start_par <- c(
        rep(0, idlist$id_len * 3), # s_ddg and b_ddg values + wildtype
        rnorm(3, mean = c(0, 0, -5.5), sd = c(1, 0.05, 1)), # binding dgwt, fwt and f0
        rnorm(2, mean = 0, sd = 1) # stability dgwt
      )
    } else if (approach == 2) {
      start_par <- c(
        fw_list$s_ddg_invitro, #set in vitro ddg's as starting point for boht s1
        fw_list$s_ddg_invitro, # and s2
    ...
    names(start_par) <- c(
      paste0(idlist$id_mut, "_s1_ddg"),
      paste0(idlist$id_mut, "_s2_ddg"),
    ...
    global_model <- optim(
      par = start_par,
      fn = function_GB1_dG_4state_fitting,
      gr = function_GB1_dG_4state_gradient,
      method = "L-BFGS-B",
      lower = lower_bounds,
      upper = upper_bounds,
