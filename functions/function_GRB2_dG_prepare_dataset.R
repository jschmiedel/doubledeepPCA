...
function_GRB2_dG_prepare_dataset <- function(
  ...
  p1 <- ggplot(all_data[Nmut == 1], aes(s_fitness)) +
    geom_density() +
    geom_vline(xintercept = c(0.025, 0.95))
  p2 <- ggplot(all_data[Nmut == 1], aes(b_fitness)) +
    geom_density() +
    geom_vline(xintercept = c(0.2, 0.95))
  ...
  X <- rbind(
    all_data[
      Nmut == 2 & !is.na(s_fitness) & !is.na(b_fitness),
      .(id = id1)
    ],
