function_GRB2_dG_4state_estimation <- function(
  ...
  if (method == 1) {
    ...
    all_data = all_data[!is.na(b_fitness)]
    all_data[, s_fitness := NA] #model 3: delete all stability fitness values
  } else if (method == 4) {
    all_data = all_data[Nmut %in% c(0,2)] #model 4: only use double mutants
  }
  ...
  fw_list <- list(
    s_fitness = all_data[test_set==F, s_fitness],
    s_sigma = all_data[test_set==F, s_sigma],
  ...
  lower_bounds <- c(
    rep(-15, idlist$s_id_len * 2 + idlist$b_id_len),
    c(-15, 0.7, -0.2),
    c(-15, -15, 0.7, -0.2)
  )
  upper_bounds <- c(
  ...
  fixed_par <- c(0, 0, 0)#,
                  # 0.15, 0.275)
  names(fixed_par) <- c("0X_s1_ddg", "0X_s2_ddg", "0X_b_ddg")#,
                        # "s_f0","b_f0")
  ...
  models <- foreach(m = 1:n_models) %dopar% {
    ...
    start_par <- c(
      rep(0, idlist$s_id_len *2 + idlist$b_id_len), # s_ddg and b_ddg values + wildtype
      rnorm(3, mean = c(0, 1, 0.275), sd = c(1, 0.05, 0.05)), # binding dgwt, fwt and f0
      rnorm(4, mean = c(0, 0 , 1, 0.15), sd = c(1, 1, 0.05, 0.05)) # stability dgwt12, fwt and f0
    )
    names(start_par) <- c(
      paste0(idlist$s_id_mut, "_s1_ddg"),
      paste0(idlist$s_id_mut, "_s2_ddg"),
      paste0(idlist$b_id_mut, "_b_ddg"),
      c("b_dgwt", "b_fwt", "b_f0"),
      c("s1_dgwt", "s2_dgwt", "s_fwt", "s_f0")
    )
    if (method == 3) {
    ...
    global_model <- optim(
      par = start_par,
      fn = function_GRB2_dG_4state_fitting,
      gr = function_GRB2_dG_4state_gradient,
      method = "L-BFGS-B",
      lower = lower_bounds,
      upper = upper_bounds,
  ...
  for (df_idx in df) {
    pll_bounds <- foreach(
      ...
      if (grepl("dd[gG]", pll_par[p, par])) { 
        ...
        record <- FALSE
      } else if (grepl("s1_dgwt", pll_par[p, par])) { 
        #it's a stability1 dgwt value
        range <- pll_par[grep("s1_ddg", par), quantile(value, c(0.01, 0.99))]
        step_size <- abs(diff(range)) * rel_stepsize
      } else if (grepl("s2_dgwt", pll_par[p, par])) { 
        #it's a stability2 dgwt value
        range <- pll_par[grep("s2_ddg", par), quantile(value, c(0.01, 0.99))]
        step_size <- abs(diff(range)) * rel_stepsize
