function_GRB2_dG_estimation <- function(
  dataset_name,
  dataset_suffix,
  method,
  maxit = 100,
...
function_GRB2_dG_estimation2 <- function(
  dataset_name,
  dataset_suffix,
  method,
  maxit = 100,
  ...
  while (max(table(best_model)) <= 10) {
    ...
    if (i == 1) {
      ...
      } else if (method %in% c(1, 2, 4)) {
        start_par <- c(
          rep(0, s_idl), # s_ddg values
          rep(0, b_idl), # b_ddg values
          rnorm(2, 0),
          rep(0.975,2)#, # wildtype dG values,
          # rnorm(1, 1, 0.1), # b_fwt parameter
          # rnorm(1, 0, 0.1), # b_f0 parameter
          # rnorm(1, 1, 0.1), # s_fwt parameter
          # rnorm(1, 0, 0.1) # s_f0 parameter
    ...
    global_model <- optim(
      ...
      s_idl = s_idl, 
      b_idl = b_idl, 
      exp_mod = as.integer(dataset_suffix)-1,
      global_par = global_par,
      dt = copy(all_data_iteration)
    )
    
    model_i <- data.table(global_model$value,t(global_model$par), t(global_par))  
    # if (method != 3) {
      names(model_i) <- c(
        "objective",
        paste0(s_id_var, "_s_ddG"),
    ...
    start_par <- models[which.min(objective), 
      unlist(.SD), 
      .SDcols = !(names(models) %in% c("objective", "b_f0", "s_f0"))]
