...
function_dG_method1_fitting <- function(parameters, id_L, global_par, list_fs) {
  ...
  if (length(parameters) == (id_L + 2)) {
    # + scale parameters [>> determine global relationship; background parameters set manually]
    # s_scale <- parameters[id_L + 1]
    # b_scale <- parameters[id_L + 2]
    # s_bgr <- global_par[1]
    # b_bgr <- global_par[2]
    # b_dG <- 0
  } else if (length(parameters) == (id_L + 4)) {
    # + scale and background parameters [>> determine global relationship]
    # s_scale <- parameters[id_L + 1]
    # b_scale <- parameters[id_L + 2]
    # s_bgr <- parameters[id_L + 3]
    # b_bgr <- parameters[id_L + 4]
    # b_dG <- 0
  # } else if (length(parameters) == (id_L + 6)) {
  } else if (length(parameters) == 6) {
    # + wildtype dG values [>> determine global relationship]
    # [use if wild-type fitness != 1]
    # s_scale <- parameters[id_L + 1]
    # b_scale <- parameters[id_L + 2]
    # s_bgr <- parameters[id_L + 3]
    # b_bgr <- parameters[id_L + 4]
    # s_Fwt <- parameters[id_L + 5]
    # b_Fwt <- parameters[id_L + 6]
    s_scale <- parameters[1]
    b_scale <- parameters[2]
    s_bgr <- parameters[3]
    b_bgr <- parameters[4]
    s_Fwt <- parameters[5]
    b_Fwt <- parameters[6]
  ...
  # determine wild-type dG from set of parameters
  s_dGwt <- function_folding_F2dG(
    s_fitness = ifelse(length(parameters) != (id_L + 6), 1, s_Fwt),
    s_bgr = s_bgr, 
    s_scale = s_scale
  )
  b_dGwt <- function_binding_F2dG(
    b_fitness = ifelse(length(parameters) != (id_L + 6), 1, b_Fwt),
    s_dG = s_dGwt, 
    b_bgr = b_bgr, 
    b_scale = b_scale
  )
...
if (length(parameters) == 6) {
  ...
  MSD <- sum((list_fs$b_fitness - bf)^2 / b_sigma^2 + 
    (list_fs$s_fitness - sf)^2 / s_sigma^2, na.rm = T) / 
    sum(b_sigma^-2 + s_sigma^-2, na.rm = T)
