...
function_folding_dG2F = function(f_dG,f_dGwt,f_bgr,f_s1,f_s2) {
  ff = (((1-f_bgr)/(1+exp((f_dGwt+f_dG)/R/Temp)) + f_bgr) - f_s1) / f_s2
}
function_binding_dG2F = function(b_dG,f_dG,b_dGwt,f_dGwt,b_bgr,b_s1,b_s2) {
  bf = (((1-b_bgr)/(1+exp((b_dGwt+b_dG)/R/Temp)*(1+exp((f_dGwt+f_dG)/R/Temp))) + b_bgr) - b_s1) / b_s2
}
function_folding_F2dG = function(f_fitness,f_bgr,f_s1,f_s2) {
  f_dG = R*Temp*log((1-f_bgr)/(f_fitness*f_s2+f_s1-f_bgr)-1)
}
function_binding_F2dG = function(b_fitness,f_dG,b_bgr,b_s1,b_s2) {
  b_dG = R*Temp*(log((1-b_bgr)/(b_fitness*b_s2+b_s1-b_bgr)-1) - log(1+exp(f_dG/R/Temp)))
}
...
function_fit_global_relationship = function(parameters) {
  ...
  f_dGwt = function_folding_F2dG(1,f_bgr,f_s1,f_s2)
  b_dGwt = function_binding_F2dG(1,f_dGwt,b_bgr,b_s1,b_s2)
  #for new data
  # f_dGwt = function_folding_F2dG(0,f_bgr,f_s1,f_s2)
  # b_dGwt = function_binding_F2dG(0,f_dGwt,b_bgr,b_s1,b_s2)
  
  
  bf = function_binding_dG2F(b_dG=0,f_dG,b_dGwt,f_dGwt,b_bgr,b_s1,b_s2)
  ff = function_folding_dG2F(f_dG,f_dGwt,f_bgr,f_s1,f_s2)
...
#new DiMSum data
# fit_global_relationship = nlminb(start = c(rep(0,L),0,0,0.6,0.125,0.6,0.125),
#                                  objective = function_fit_global_relationship,
#                                  lower = c(rep(-Inf,L),0,0,0,0,0,0),
#                                  upper = c(rep(Inf,L),1,1,1,1,1,1),
#                                  control = list(iter.max = 200))
print(fit_global_relationship)
...
table_global_parameters = data.table(b_bgr = fit_global_relationship$par[L+1],
                                     ...
                                     b_s2 = fit_global_relationship$par[L+4],
                                     f_s1 = fit_global_relationship$par[L+5],
                                     f_s2 = fit_global_relationship$par[L+6])

#calcualte again dG wildtype values from final parameters
#old
f_dGwt = function_folding_F2dG(1,table_global_parameters$f_bgr,table_global_parameters$f_s1,table_global_parameters$f_s2)
b_dGwt = function_binding_F2dG(1,f_dGwt,table_global_parameters$b_bgr,table_global_parameters$b_s1,table_global_parameters$b_s2)
#new DiMSum data
...
DT[is.global==T,f_fitness_global := function_folding_dG2F(f_ddG_global,f_dGwt,
                                                          table_global_parameters$f_bgr,table_global_parameters$f_s1,table_global_parameters$f_s2),f_ddG_global]
DT[is.global==T,b_fitness_global := function_binding_dG2F(0,f_ddG_global,b_dGwt,f_dGwt,
                                                          table_global_parameters$b_bgr,table_global_parameters$b_s1,table_global_parameters$b_s2),f_ddG_global]
...
DT[,f_fitness_local := function_folding_dG2F(f_ddG_local,f_dGwt,
                                             table_global_parameters$f_bgr,table_global_parameters$f_s1,table_global_parameters$f_s2),f_ddG_local]
DT[,b_fitness_local := function_binding_dG2F(0,f_ddG_local,b_dGwt,f_dGwt,
                                             table_global_parameters$b_bgr,table_global_parameters$b_s1,table_global_parameters$b_s2),f_ddG_local]

DT2 = copy(DT)
#adjust for wild-type dG
# DT2[,f_ddG_global := table_global_parameters$f_dGwt + f_ddG_global]
# DT2[,f_ddG_local := table_global_parameters$f_dGwt + f_ddG_local]
# DT2[,b_ddG_local := table_global_parameters$b_dGwt + b_ddG_local]
X=melt(DT2,measure.vars = grep("ddG",names(DT),value=T))
X[,variable := factor(variable,levels=c("f_ddG_global","f_ddG_local","b_ddG_local"))]
ggplot(X) +
...
#pleiotropy
DT[,pleiotropy := "none"]
DT[b_ddG_local > 0 & f_ddG_local > 2,pleiotropy := "b_ddG > 0 & f_ddG > 2"]
DT[b_ddG_local > 1.2 & f_ddG_local > 3,pleiotropy := "b_ddG > 1.2 & f_ddG > 3"]
p1=ggplot(DT) + 
