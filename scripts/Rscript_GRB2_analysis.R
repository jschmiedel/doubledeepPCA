...
singles = fread(file = "processed_data/GRB2_singles_alldata.txt")
# exp fitness values
...
fig2 <- grid.arrange(p2A, p2B, layout_matrix = rbind(c(1,1,1,2,2),
                                                     c(1,1,1,2,2)))
ggsave("results/GRB2_fig2_assembled.pdf", fig2, width = 9, height = 3.5)
...
ggsave(plot=p3A,file = "results/GRB2_fig3_panelA.pdf",height=unit(9,"cm"),width=unit(7,"cm"))


### panel B: GRB2-GAB2 crystal structure residues coloured by fitness or fitness difference
# create txt file to run in pymol

transCM = fread("dataset/GRB2/PDB_contactmap_2vwf_AB.txt")                                                                              
singlesCM = merge(singles,transCM[,.(trans_dist = min(scHAmin)),Pos1],by.x="Pos",by.y="Pos1")
singlesCM_avg = singlesCM[,.(diff_binding_stability = mean(b_fitness_exp-s_fitness_exp),
                             b_fitness_exp = mean(b_fitness_exp),
                             trans_dist = unique(trans_dist), 
                             s_fitness_exp = mean(s_fitness_exp)),Pos]

script_file = "results/pymol/GRB2_Fig3_panelB.txt"
...
for (i in 1:nrow(singlesCM_avg)) {
  pymol_script[length(pymol_script)+1]  = paste0("alter 2vwf and chain A and resid ",i,", b=",singlesCM_avg[i,b_fitness_exp])
}
...
for (i in 1:nrow(singlesCM_avg)) {
  pymol_script[length(pymol_script)+1]  = paste0("alter 2vwf and chain A and resid ",i,", b=",singlesCM_avg[i,s_fitness_exp])
}
pymol_script[length(pymol_script)+1] = 'spectrum b, red_white, chain A, minimum=0.5, maximum=1'
pymol_script[length(pymol_script)+1] = "ray 2400,2400"
...
singles_min = singles[is.finite(b_fitness_exp),.(diff = min(diff_binding_stability,na.rm=T),
                                           mindist = min(HAmin_ligand),
                                           stab = min(s_fitness_exp,na.rm=T),
                                           bind = min(b_fitness_exp,na.rm=T)),.(Pos,RSA_unbound)]

# average fitness or delta fitness
singles_avg = singles[,.(meandist = mean(HAmin_ligand),
                         diff = mean(diff_binding_stability,na.rm=T),
                         mindist = min(HAmin_ligand),
                         stab = mean(s_fitness_exp,na.rm=T),
                         dist5A = min(HAmin_ligand) < threshold_ligand_dist,
                         bind = mean(b_fitness_exp,na.rm=T), 
                         pval_bind = p.adjust(t.test(b_fitness_exp, mu=1, alternative="less")$p.val, method = "fdr"),
                         pval_stab = p.adjust(t.test(s_fitness_exp, mu=1, alternative="less")$p.val, method = "fdr"),
...
panel_4A = ggplot(Y) +
  ...
  labs(x="average fitness per residue",y="minimal distance to GAB2",color="FDR < 0.05",fill="distance < 5??")
panel_4A
ggsave("results/GRB2_fig4_panelA.pdf", panel_4A, width = 8.5, height = 3)
...
panel_4B = grid.arrange(panel_4B_1, panel_4B_2, layout_matrix = rbind(c(1,1),
                                                                      c(1,1),
                                                                      c(2,2)))
ggsave("results/GRB2_fig4_panelB.pdf", panel_4B, width = 3.5, height = 4.5)
...
## Per mutation
singles[is.finite(b_fitness_exp) & is.finite(s_fitness_exp),.N] #total of 504 mutations
# bindingPCA vs stabilityPCA
singles[is.finite(b_fitness_exp) & is.finite(s_fitness_exp),.(total=.N,freq = .N/singles[is.finite(b_fitness_exp) & is.finite(s_fitness_exp),.N]),.(affects_binding,affects_stability)]

# Binding affinity only using delta fitness score
singles[is.finite(b_fitness_exp) & is.finite(s_fitness_exp),.(total=.N,freq = .N/singles[is.finite(b_fitness_exp) & is.finite(s_fitness_exp),.N]),.(affects_binding_biochem,affects_stability)]
