...
# ggsave("dataset/preprocessing/GRB2_CYC_sub_indel_frequency.pdf")
...
if (doplot == T) {
  ...
  ggsave("dataset/preprocessing/GRB2_CYC_seqread_distributions.pdf",width=7,height=7)
}
...
if (doplot == T) {
  ggplot(dataset,aes((output1_e1_s1_b1_count+0.001)/(input1_e1_s0_bNA_count+0.001),..scaled..,color=factor(Nmut_nt))) +
    ...
    facet_grid(Nmut_aa ~ .) +
    labs(x="fitness")
  ggsave("dataset/preprocessing/GRB2_CYC_fitness_higherorder_ntmut.pdf",width=7,height=7)
...
if (doplot == T) {
  ...
  ggplot(melt(dataset_syn[Nmut_aa > 0],id.vars = c("aa_seq","Nmut_aa","Nmut_nt","WT","STOP")),aes(value,color=factor(Nmut_aa))) +
    ...
    scale_x_log10() +
    facet_wrap(variable ~ .)
  ggsave("dataset/preprocessing/GRB2_CYC_seqread_syn_distributions.pdf",width=7,height=7)
}
...
if (doplot == T) {
  ...
  ggsave("dataset/preprocessing/GRB2_CYC_read_fitness_2dhex.pdf",width=7,height=7)
  #severe limitations from low input counts on fitness range
  ggplot(DT,aes(fitness,sigma)) +
    geom_hex() +
    scale_y_log10() +
    facet_wrap( ~ rep)
  ggsave("dataset/preprocessing/GRB2_CYC_fitness_sigma_2dhex.pdf",width=7,height=7)
}
...
if (doplot == T) {
  ...
  ggsave("dataset/preprocessing/GRB2_CYC_fitness_merged_uncorrVScond.pdf",width=7.5,height=7.5)
}
...
write.table(x = wildtype, file = "dataset/GRB2_CYC_wildtype.txt",
            quote = F,row.names = F, col.names = T)
write.table(x = singles, file = "dataset/GRB2_CYC_singles_readT20.txt",
            quote = F,row.names = F, col.names = T)
write.table(x = doubles, file = "dataset/GRB2_CYC_doubles_readT20.txt",
            quote = F,row.names = F, col.names = T)
