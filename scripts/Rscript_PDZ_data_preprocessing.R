...
# wildtype
wildtype_NM_stabilityPCA <- all_variants[
  WT == T,
  .(
    fitness = sum(fitness * sigma^-2) / sum(sigma^-2),
    sigma = sqrt(1 / sum(sigma^-2)),
...
singles_NM_stabilityPCA <- all_variants[
  Nham_aa == 1 & Nmut_codons == 1 & ((Nham_nt == 1 & mean_count > 100) | (Nham_nt > 1 & mean_count > 10)),
  .(
    fitness = sum(fitness * sigma^-2) / sum(sigma^-2),
    sigma = sqrt(1 / sum(sigma^-2)),
    mean_count = sum(mean_count), Nsyn = .N,
...
# # if Nham_nt > 1, variants below 10^1 read counts are sequencing errors
# # also there's a proportionality between readcounts of the same AA variants that could be taken into account
wildtype_NM_bindingPCA <- all_variants[
  WT == T,
  .(
    fitness = sum(fitness * sigma^-2) / sum(sigma^-2),
    sigma = sqrt(1 / sum(sigma^-2)),
...
## merge based on hamming-distance specific read thresholds
singles_NM_bindingPCA <- all_variants[
  Nham_aa == 1 & Nmut_codons == 1 & ((Nham_nt == 1 & mean_count > 100) | (Nham_nt > 1 & mean_count > 10)),
  .(
    fitness = sum(fitness * sigma^-2) / sum(sigma^-2),
    sigma = sqrt(1 / sum(sigma^-2)),
...
wildtype_epPCR_stabilityPCA <- all_variants[
  WT == T,
  .(
    fitness = sum(fitness * sigma^-2) / sum(sigma^-2),
    sigma = sqrt(1 / sum(sigma^-2)),
...
wildtype_epPCR_bindingPCA <- all_variants[
  Nham_nt == 0,
  .(
    fitness = sum(fitness * sigma^-2) / sum(sigma^-2),
    sigma = sqrt(1 / sum(sigma^-2)),
...
wildtype <- merge(
  merge(
    ...
    wildtype_NM_bindingPCA[, .(
      aa_seq, 
      STOP, 
      STOP_readthrough,
      mean_count_bNM = mean_count,
      b_fitness_NM = fitness, 
      b_sigma_NM = sigma
    )],
    all = T
    ),
...
set.seed(1603)
minF <- function(p) {
  F_norm <- (F_data + matrix(
    p[(Nsamples + 1):(2 * Nsamples)], 
    nrow = nrow(F_data), 
    ncol = Nsamples, 
    byrow = T
  )) * matrix(
...
F_data <- singles[mean_count_sNM > 100 & mean_count_sEP > 100, 
  as.matrix(.SD), 
  .SDcols = grep("s_fitness_(NM|EP)$", names(singles))]
Nsamples <- 2
x <- nlm(minF, rep(c(1, 0), each = Nsamples))
print(x)
p <- x$estimate
fitness_norm_model <- data.table(t(p))
names(fitness_norm_model) <- c(
  "scale_sNM", 
  "scale_sEP", 
  "shift_sNM", 
  "shift_sEP"
...
wt_corr <- wildtype[, 
  rowMeans((.SD + unlist(fitness_norm_model[, 
    .SD, ,
    .SDcols = grep("shift_s", names(fitness_norm_model))])) *
...
wildtype[, s_fitness_NM_norm := (s_fitness_NM + fitness_norm_model$shift_sNM) * fitness_norm_model$scale_sNM - wt_corr]
wildtype[, s_fitness_EP_norm := (s_fitness_EP + fitness_norm_model$shift_sEP) * fitness_norm_model$scale_sEP - wt_corr]
wildtype[, s_sigma_NM_norm := s_sigma_NM * fitness_norm_model$scale_sNM]
wildtype[, s_sigma_EP_norm := s_sigma_EP * fitness_norm_model$scale_sEP]
ggplot(
...
wildtype[, s_fitness := sum(c(s_fitness_NM_norm / s_sigma_NM_norm^2, s_fitness_EP_norm / s_sigma_EP_norm^2), na.rm = T) / sum(c(s_sigma_NM_norm^-2, s_sigma_EP_norm^-2), na.rm = T)]
wildtype[, s_sigma := sqrt(1 / sum(c(s_sigma_NM_norm^-2, s_sigma_EP_norm^-2), na.rm = T))]
...
## normalize fitness values
singles[, b_fitness_NM_norm := (b_fitness_NM + fitness_norm_model$shift_bNM) * 
  fitness_norm_model$scale_bNM - wt_corr]
singles[, b_fitness_EP_norm := (b_fitness_EP + fitness_norm_model$shift_bEP) * 
  fitness_norm_model$scale_bEP - wt_corr] 
...
wildtype[, b_fitness_NM_norm := (b_fitness_NM + fitness_norm_model$shift_bNM) * 
  fitness_norm_model$scale_bNM - wt_corr]
wildtype[, b_fitness_EP_norm := (b_fitness_EP + fitness_norm_model$shift_bEP) * 
  fitness_norm_model$scale_bEP - wt_corr] 
wildtype[, b_sigma_NM_norm := b_sigma_NM * fitness_norm_model$scale_bNM]
wildtype[, b_sigma_EP_norm := b_sigma_EP * fitness_norm_model$scale_bEP]
...
#### merge fitness and error
wildtype[, b_fitness := sum(c(
    b_fitness_NM_norm / b_sigma_NM_norm^2, 
    b_fitness_EP_norm / b_sigma_EP_norm^2
  ), na.rm = T) / 
  sum(c(
    b_sigma_NM_norm^-2, 
    b_sigma_EP_norm^-2
...
wildtype[, b_sigma := sqrt(1 / sum(c(
    b_sigma_NM_norm^-2, 
    b_sigma_EP_norm^-2
  ), na.rm = T))
]

singles[, b_fitness := sum(c(
    b_fitness_NM_norm / b_sigma_NM_norm^2, 
    b_fitness_EP_norm / b_sigma_EP_norm^2
  ), na.rm = T) / 
  sum(c(
...
# plot comparision; also to McLaughlin & Riesselman data
# mclaughlin = fread("dataset/PDZ/McLaughlin2012_DLG4_singlemutants.txt")
mclaughlin <- fread("dataset/PDZ/deepSequence_PDZ_predictions.txt")
...
singles_RN <- merge(singles,
  mclaughlin[, .(
    Pos,
    Mut,
    CRIPT,
    Tm2F,
    pred_vae = mutation_effect_prediction_vae_ensemble,
    pred_pairwise = mutation_effect_prediction_pairwise,
    pred_ind = mutation_effect_prediction_independent
  )],
  by = c("Pos", "Mut")
)
singles_RN[, cor(.SD, use = "pairwise.complete.obs", method = "spearman"), .SDcols = c(
  "s_fitness",
  "b_fitness",
  "CRIPT",
  "Tm2F",
...
mclaughlin[, cor(.SD, use = "pairwise.complete.obs"), .SDcols = c("CRIPT", "Tm2F", grep("mutation", names(mclaughlin), value = T))]
ggpairs(singles_RN,
  columns = c(
    "s_fitness",
    "b_fitness",
...
pairdistances_from_PDB(
  input_file = "dataset/PDZ/PDB/pdb1be9.ent", dataset_dir = "",
...
### >>save data files
write.table(wildtype, file = "processed_data/PDZ_wildtype_alldata.txt", quote = F, row.names = F)
write.table(singles, file = "processed_data/PDZ_singles_alldata.txt", quote = F, row.names = F)