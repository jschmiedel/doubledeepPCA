...
#### NM stability ####
######################
all_variants = fread("dataset/GRB2/01c-GRB2_NM2_stabilityPCA/01c-GRB2_NM2_stabilityPCA_variant_data_merge.tsv")
# normalize by generations
gen = c(5, 5.1, 5.4)
time = c(33.47, rep(32.25, 2))
...
# zeros_NM_stabilityPCA <- all_variants[
#   Nham_aa == 0 & Nham_nt == 1, #& 
#     # ((Nham_nt == 1 & mean_count > 10^3) | (Nham_nt == 2 & mean_count > 10^2)),
#   .(
#     fitness = sum(fitness * sigma^-2) / sum(sigma^-2),
#     sigma = sqrt(1 / sum(sigma^-2)),
#     mean_count = sum(mean_count), 
...
singles_NM_stabilityPCA <- all_variants[
  Nham_aa == 1 &  
    ((Nham_nt == 1 & mean_count > 10^3) | (Nham_nt > 1 & mean_count > 10^2)),
  .(
    fitness = sum(fitness * sigma^-2) / sum(sigma^-2),
    sigma = sqrt(1 / sum(sigma^-2)),
    mean_count = sum(mean_count), 
...
#### epPCR stability ###
########################
all_variants = fread("dataset/GRB2/01a-GRB2_epPCR_stabilityPCA/01a-GRB2_epPCR_stabilityPCA_variant_data_merge.tsv")
# normalize by generations
gen = c(5.19, 5.07, 5.1)
time = rep(34.1, 3)
...
# zeros_epPCR_stabilityPCA <- all_variants[
#   Nham_aa == 0 & Nham_nt == 1 & Nmut_codons == 1,
#   .(
#     fitness = sum(fitness * sigma^-2) / sum(sigma^-2),
#     sigma = sqrt(1 / sum(sigma^-2)),
#     mean_count = sum(mean_count), 
...
#### epPCR binding ####
#######################
all_variants = fread("dataset/GRB2/01b-GRB2_epPCR_bindingPCA/01b-GRB2_epPCR_bindingPCA_variant_data_merge.tsv")
...
# zeros_epPCR_bindingPCA <- all_variants[
#   Nham_aa == 0 & Nmut_codons == 1 & (Nham_nt == 1),
#   .(
#     fitness = sum(fitness * sigma^-2) / sum(sigma^-2),
#     sigma = sqrt(1 / sum(sigma^-2)),
#     mean_count = sum(mean_count),
...
ggplot(singles, aes(s_fitness_NM, s_fitness_EP)) +
  geom_density2d() +
  geom_point() +
  geom_smooth() +
  geom_abline()
...
ggplot(doubles, aes(s_fitness_NM, s_fitness_EP)) +
  geom_density2d() +
  # geom_point() +
  geom_smooth() +
  geom_abline()
ggsave("results/preprocessing/GRB2_stabilityPCA_fitness_comparision2.pdf", width = 8, height = 8)
...
wildtype[, s_fitness_NM_norm := 
  (s_fitness_NM + fitness_norm_model$shift_sNM) * 
  fitness_norm_model$scale_sNM - wt_corr + 1]
wildtype[, s_fitness_EP_norm := 
  (s_fitness_EP + fitness_norm_model$shift_sEP) * 
  fitness_norm_model$scale_sEP - wt_corr + 1]
...
ggplot(singles, aes(s_fitness_NM_norm, s_fitness_EP_norm)) +
  geom_density2d() +
  geom_point() +
  geom_smooth(method = "loess", span = 0.5) +
  geom_abline()
ggsave("results/preprocessing/GRB2_stabilityPCA_fitness_comparision3.pdf", width = 8, height = 8)
...
# ggsave(filename = "results/preprocessing/GRB2_sfitness_merge.pdf", width = 6, height = 6)
# this is caused by low count variants, so it's fine

# merge fitness and error
wildtype[, s_fitness := sum(c(s_fitness_NM_norm / s_sigma_NM_norm^2, 
