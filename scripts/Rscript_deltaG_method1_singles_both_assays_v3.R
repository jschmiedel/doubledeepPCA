...
#fitness to state probability transformation b_s1,b_scale,f_s1 and f_scale
...
X[,ff:=function_folding_dG2F(f_dG=f_dg,f_dGwt=-1.3,f_bgr=0.35,f_scale=1),f_dg]
X[,bf:=function_binding_dG2F(0,f_dg,0,-1.3,0.2,0.8),f_dg]
...
### determine background growth
# ggplot(DT,aes(binding)) +
#   geom_density() +
#   geom_vline(xintercept = 0.3)
# 
# ggplot(DT,aes(stability)) +
#   geom_density() +
#   geom_vline(xintercept = 0.35)
# 
# ggplot(DT,aes(binding,stability)) +
#   geom_density2d() +
#   geom_point() +
#   geom_vline(xintercept = 0.3) +
...
function_fit_global_relationship = function(parameters) {
  ...
  # b_scale = parameters[L+3]
  # f_scale = parameters[L+4]
  
  
  b_scale = parameters[L+1]
  f_scale = parameters[L+2]
  ...
  #for new data
  # f_dGwt = function_folding_F2dG(0,f_bgr,f_s1,f_scale)
  # b_dGwt = function_binding_F2dG(0,f_dGwt,b_bgr,b_s1,b_scale)
  
  
  bf = function_binding_dG2F(b_dG=0,f_dG,b_dGwt,f_dGwt,b_bgr,b_scale)
  # bf = function_binding_dG2F(b_dG,f_dG,b_dGwt,f_dGwt,b_bgr,b_scale)
  ff = function_folding_dG2F(f_dG,f_dGwt,f_bgr,f_scale)
  ...
  # MSD = sum((binding_fitness2 - bf)^2 / binding_error2 + (stability_fitness2 - ff)^2 / stability_error2,na.rm=T) / sum(binding_error2 + stability_error2,na.rm=T)
